...
class Ui:
    ...
    def _setup_widgets(self):
        ...
        self.card_number.grid(column=0, row=2)
        self.box_status.grid(column=4, row=2)

        self.question_canvas = tkinter.Canvas(width=300, height=90)
        self.question_canvas.grid(column=0, columnspan=5, row=3, pady=5, padx=5)
        ...
        self.answer_button.grid(column=0, columnspan=5, row=4)
    ...
    def show_answer(self):
        if not self.answer_visible:
            ...
            self.answer_canvas.grid(column=0, columnspan=5, row=5, pady=5, padx=5)
            self.divider2.grid(column=0, row=6, columnspan=5, pady=10, sticky='ew')
            self.correct_button.grid(column=0, columnspan=2, row=7)
            self.incorrect_button.grid(column=4, columnspan=2, row=7)
    ...
    def on_correct(self):
        if self.progress[self.current_card.id] < 5:
            self.progress[self.current_card.id] += 1
            print(f"✅ 정답! → Box {self.progress[self.current_card.id]}")
        else:
